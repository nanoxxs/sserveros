...
Agent 默认关闭，只有在设置中启用后才显示入口：电脑端显示右下角 AI 图标并打开浮窗，手机端显示独立的 Agent 标签页。它可以调用只读工具查询当前 GPU 快照、监控 PID、进程、systemd 服务、端口、磁盘和系统信息；涉及写操作的 `add_watch_pid` / `remove_watch_pid` 会先进入待确认状态，需要在对话界面中确认后才会真正写入配置并通知 `monitor.py`。
